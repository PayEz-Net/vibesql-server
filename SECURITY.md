...
`scripts/base-schema.v2.sql` (a file that exists only on feature branches, not on `master`) as it existed on branch `fix/wire-audit-log-repository`
...
```
# the probe lives on branch fix/wire-audit-log-repository:
